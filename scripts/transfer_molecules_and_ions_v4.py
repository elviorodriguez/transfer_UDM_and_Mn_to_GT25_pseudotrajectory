...
def find_structurally_conserved_core(ca_fixed, ca_moving, residue_pairs, 
    ...
    """
    Identify structurally conserved core regions using a more lenient pruning strategy.
    
    Parameters:
    - distance_cutoff: Maximum distance (in Å) for an atom pair to be considered aligned
    - min_core_size: Minimum number of atoms to keep in the core
    - percentile_cutoff: Keep atoms below this percentile of distances
    """
    import numpy as np
    
    if len(ca_fixed) < min_core_size:
        print(f"  Warning: Only {len(ca_fixed)} aligned atoms, less than minimum {min_core_size}")
        if len(ca_fixed) < 10:
    ...
    # Iterative refinement with lenient pruning
    max_iterations = 5
    for iteration in range(max_iterations):
        if len(current_fixed) < min_core_size:
            break
            
        super_imposer.set_atoms(current_fixed, current_moving)
        rot, tran = super_imposer.rotran
        
        # Calculate distances after current alignment
        distances = []
        ...
        # Use a lenient cutoff: keep the best X% of atom pairs
        cutoff = min(np.percentile(distances, percentile_cutoff), distance_cutoff)
        ...
        if len(new_fixed) < min_core_size:
            break
        ...
        print(f"  Iteration {iteration + 1}: {len(current_fixed)} atoms, cutoff={cutoff:.2f}Å, mean_dist={np.mean(distances[keep_indices]):.2f}Å")
...
def main():
    ...
    parser.add_argument('--distance-cutoff', type=float, default=5.0,
                        help='Maximum distance cutoff for alignment (Å) (default: 5.0)')
    parser.add_argument('--min-core-size', type=int, default=30,
                        help='Minimum number of atoms in aligned core (default: 30)')
    parser.add_argument('--percentile', type=float, default=80,
                        help='Percentile cutoff for keeping atom pairs (default: 80)')
    ...
    print(f"Parameters: distance_cutoff={args.distance_cutoff}Å, min_core_size={args.min_core_size}, percentile={args.percentile}")
    ...
    for model_idx, traj_model in enumerate(trajectory_structure):
        ...
        # Find structurally conserved core
        ...
        print(f"  Final alignment: {len(core_fixed)} CA atoms ({len(core_fixed)/len(ca_fixed)*100:.1f}% of aligned)")
