...
def iterative_pruning_alignment(ca_fixed, ca_moving, residue_pairs, cutoff=2.0, iterations=5):
    """
    Perform iterative pruning similar to ChimeraX Matchmaker.
    Remove atom pairs with high deviation and re-align.
    """
    import numpy as np
    ...
    for iteration in range(iterations):
        ...
        # Calculate distances for each atom pair after alignment
        ...
        # Calculate cutoff based on RMSD (similar to ChimeraX)
        mean_dist = np.mean(distances)
        std_dist = np.std(distances)
        dynamic_cutoff = mean_dist + cutoff * std_dist
        
        # Keep only pairs below cutoff
        keep_indices = distances < dynamic_cutoff
    ...
    # Final alignment with pruned atoms
    if len(current_fixed) >= 3:
        super_imposer.set_atoms(current_fixed, current_moving)
    
    return super_imposer, len(current_fixed), len(ca_fixed)
...
def main():
    ...
    for model_idx, traj_model in enumerate(trajectory_structure):
        ...
        print(f"  Initial alignment: {len(ca_fixed)} CA atom pairs")
        
        # Perform iterative pruning to improve alignment (like ChimeraX Matchmaker)
        super_imposer, pruned_count, total_count = iterative_pruning_alignment(
            ca_fixed, ca_moving, residue_pairs, cutoff=2.0, iterations=5
        )
        
        rmsd = super_imposer.rms
        print(f"  After pruning: {pruned_count}/{total_count} CA atoms kept")
